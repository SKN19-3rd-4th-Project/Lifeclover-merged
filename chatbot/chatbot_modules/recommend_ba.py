...
def recommend_activities_tool(user_emotion: str, mobility_status: str = "거동 가능") -> str:
    ...
    - user_emotion: "우울", "무기력", "심심" 등 사용자의 감정 요약 문자열
    - mobility_status: "거동 가능", "와상", "보행 보조 필요" 등 이동 가능 상태
    """
    ...
    for m in selected_matches:
        meta = m["metadata"]
        results.append(
            f"- {meta.get('activity_kr')} (기대효과: {meta.get('FEELING_TAGS')})"
        )
...
# 외부 모듈에서 import 할 수 있도록 공개 리스트 정의
